...
testthat::skip_if_not_installed(testthat::test_that( "Survival fit computed correctly", {
    expected <- c(2,1,1,2)
...
testthat::skip_if_not_installed(testthat::test_that( "It throw an error if argument status is missing for survival", {
  expect_error(survclustcurves(time = veteran$time, x = veteran$celltype,
                             algorithm = 'kmeans'))
}))

testthat::skip_if_not_installed(testthat::test_that( "Argument status must be a vector of binary numbers", {
  expect_error(survclustcurves(time = veteran$time, x = veteran$celltype,
...
testthat::skip_if_not_installed(testthat::test_that( "It throw an error if argument time is missing", {
