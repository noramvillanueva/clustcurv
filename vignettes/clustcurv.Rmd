...
date: "`r Sys.Date()`"
description: >
  This vignette describes the usage of the main functions included in clustcurv package with three real data sets.
...
This document explains how to use  [clustcurv](https://cran.r-project.org/package=clustcurv) R package for clustering multiple nonparametric curves, under the survival, the regression and competing risks framework. To this end, we illustrate the use of the package using some real data sets. In the case of the survival context, the algorithm to determine groups automatically  is applied to Veterans' Administration Lung Cancer Data [survival](https://cran.r-project.org/package=survival) package. For  the regression analysis, the  clustcurv R package includes a data set called  `data(barnacle5)` with measurements of rostro-carinal length and dry weight of barnacles collected from five sites of Galicia (northwest of Spain). For the cumulative incidence functions we illustrate the usefulness of the methods using data from leukemia patients from the EBMT [mstate](https://cran.r-project.org/package=mstate) package.
...
# Clustering multiple cumulative incidences functions

The data set contains 8966 patients with acute myeloid leukemia, acute lymphoblastic leukemia, or chronic myeloid leukemia who received this type of transplant.


Patients who undergo bone marrow transplantation (BMT) or other types of hematopoietic stem cell transplantation (HSCT) are at increased risk of developing a variety of complications that compete with each other, including relapse of the underlying disease, graft-versus-host disease (GvHD), and different kinds of infections like bacterial, viral, and fungal. Other causes of death in this data set are gathered in the category ``other''.  
...
When we confront a data set like this one, with several competing risks, one may wish to know if the risk in patients can be influenced by the type of death. Maybe a good approximation could be to establish groups with the same risk or CIF. In order to do this, our proposed method is applied by means of executing the following piece of code
...
The results suggest that it is possible to create four groups of CIFs. The first one is GvHD with the highest risk after BMT. The second one considers other causes and the third one is the relapse of the underlying disease, which risk becomes the highest  25 days after the medical intervention. Finally, different kinds of infections—Bacterial, Viral, and Fungal—are assigned to the same group (fourth one) with the lowest probability of risk, which is nearly constant over time a few days after the intervention. 



The assignment of the curves to the four groups can be observed in the following figure grouped by colors.
