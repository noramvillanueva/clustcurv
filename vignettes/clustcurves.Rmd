...
This document explains how to use clustcurv [clustcurv](https://cran.r-project.org/package=clustcurv) R package for clustering multiple 
nonparametric curves, under the survival and regression framework. To this end, we
illustrate the use of clustcurv R package using some real data sets. In the case
of the survival context, the algorithm to determine groups automatically  were 
...
collected from five sites of the northwest of Spain. 
...
Estimates of the survival curves are obtained using the `ksurvcurves()` function, or equivalently, `survclustcurves()`. The main difference between them is that `ksurvcurves()`, given a fixed value of $K$, allows determing the group for which each survival function belongs. By  contrast,  `survclustcurves()` is able to determine automatically the number of groups. The functions will verify if data has been introduced correctly and will create  `kcurves` and `clutcurves` objects, respectively. Both functions allow determining groups using the optimization algorithm $K$-means or $K$-medians (e.g. `algorithm = 'kmeans'`, or `algorithm = 'kmedians'`). The first three arguments must be introduced, where `time` is a vector with event-times, `status` for their corresponding indicator statuses, and `x` is the categorical covariate. 


By means of the `ksurvcurves()` function and including, for example, the arguments `k = 3` and `algorithm = 'kmedians'`, the estimates and the group for which each survival function belongs can be obtained as follows, 

```{r}
fit.kgbcs<- ksurvcurves(time = gbcsCS$rectime, status = gbcsCS$censrec, x = gbcsCS$nodes,
                   algorithm = 'kmedians', k = 3, seed = 300716)
```

Additionally, one can be interesting to know, not only, the assignment of the survival curves to the group which they belong but also, the automatic selection of their number. As we mentioned, it is possible by means of the `survclustcurves()`function. The following input commands provide an example of the output using, as well, the $K$-medians algorithm (i.e. `algorithm = 'kmedians'`)

```{r}
fit.gbcs <- survclustcurves(time = gbcsCS$rectime, status = gbcsCS$censrec, x = gbcsCS$nodes,
                     nboot = 500, seed = 300716, algorithm = 'kmedians', cluster = TRUE)
``` 
...
The following piece of code can be executed for using the `summary()` function. 
```{r}
summary(fit.kgbcs)
...
Equivalently, the following piece of code shows the input commands and the results obtained with the `algorithm = 'kmeans'`. However, the number of groups and the assignments are different as those ones obtained with the `algorithm = 'kmedians'`. Although this situation is not so common, in some real applications it can happen.
```{r}
fit.gbcs2 <- survclustcurves(time = gbcsCS$rectime, status = gbcsCS$censrec, seed = 300716,
                      x = gbcsCS$nodes, algorithm = 'kmeans', cluster = TRUE, nboot = 500)
```
...
Equivalent to the example shown before with survival curves, the results obtained above can be plotted using the `autoplot()`  with the argument `interactive = TRUE` for an interactive graphic, as follows 
